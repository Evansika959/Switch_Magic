...
def plot_confidence_map(data, filename="confidence_map", title="Heatmap of Confidence in Encoder Blocks"):
    ...
    plt.title(title, fontsize=24)  # Set the title of the heatmap
    plt.xlabel("Attention Heads", fontsize=22)
    ...
    plt.xticks(ticks=range(len(data[0])), labels=[f'Head {i}' for i in range(len(data[0]))], fontsize=18)  # Set x-axis ticks
    plt.yticks(ticks=range(len(data)), labels=[f'Layer {i}' for i in range(len(data))], fontsize=18)  # Set y-axis ticks
    plt.tight_layout()
